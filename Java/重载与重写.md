...
可以看到当我们调用`getDeclaredMethods`方法时，返回的方法中居然包含一个`Object get()`方法。这是一个很奇怪的地方，因为我们知道`getDeclaredMethods`方法获取的是类自身声明的所有方法，包含public、protected和private方法，但是不包含从基类基础的、从接口实现的方法。
