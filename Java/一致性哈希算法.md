...
public void testDist() {
    ...
    Hash hash = new Hash(nodesSize);
    for (int num : nums) {
        hash.add(String.valueOf(num));
    }
    hash.statistics();
}
```
...
public void testMigrate() {
    ...
    Hash hash = new Hash(nodesSize);
    Hash hashInc = new Hash(nodesSizeInc);
    Hash hashDec = new Hash(nodesSizeDec);
...
public void testMigrate() {
    ...
    ConsistHash hash = new ConsistHash(nodesSize).initNodes();
    ConsistHash hashInc = new ConsistHash(nodesSizeInc).initNodes();
    ConsistHash hashDec = new ConsistHash(nodesSizeDec).initNodes();
...
public void testDist() {
    ...
    int[] nums = RandomNum.getRandomNum(SIZE * nodesSize);
    logger.info("start");
    VirtualConsistHash virtualConsistHash = new VirtualConsistHash(nodesSize, 100).initNodes();
    for (int num : nums) {
    ...
    ConsistHash consistHash = new ConsistHash(nodesSize).initNodes();
    for (int num : nums) {
...
public void testMigrate() {
    ...
    VirtualConsistHash hash = new VirtualConsistHash(nodesSize, 100).initNodes();
    VirtualConsistHash hashInc = new VirtualConsistHash(nodesSizeInc, 100).initNodes();
    VirtualConsistHash hashDec = new VirtualConsistHash(nodesSizeDec, 100).initNodes();
